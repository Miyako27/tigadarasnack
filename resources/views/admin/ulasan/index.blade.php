...
@section('content')
    ...
    <div class="card border-0 shadow mb-4">
        <div class="card-body">
            <div class="mb-3">
                <form method="GET" action="{{ route('ulasan.list') }}">
                    <div class="row">
                        <div class="col-md-4">
                            <!-- Input Pencarian -->
            ...
            <div class="table-responsive">
                <table class="table table-centered table-nowrap mb-0 rounded">
                    ...
                    <tbody>
                        ...
                        @foreach ($dataUlasan as $row)
                            <tr>
                                <td>{{ ($dataUlasan->currentPage() - 1) * $dataUlasan->perPage() + $loop->iteration }}
                                </td>
                                <td>{{ $row->nama_ulasan }}</td>
                                <td>{{ $row->keterangan_ulasan }}</td>
                                <td>{{ $row->ulasan }}</td>
                                <td>
                                    <a href="{{ route('ulasan.edit', $row->ulasan_id) }}" class="btn btn-warning btn-sm">
                                    ...
                                    <a href="{{ route('ulasan.destroy', $row->ulasan_id) }}" class="btn btn-danger btn-sm">
                                        <svg class="icon icon-xs me-2" data-slot="icon" fill="none" stroke-width="1.5"
                                            stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"
                                            aria-hidden="true">
                                            <path stroke-linecap="round" stroke-linejoin="round"
                ...
                <div class="mt-3">
                    {{ $dataUlasan->links('pagination::bootstrap-5') }}
                </div>
            </div>
        </div>
    </div>
