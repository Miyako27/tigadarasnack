...
@section('content')
    ...
    <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center py-4">
        <div class="d-block mb-4 mb-md-0">
            ...
            <p class="mb-0">List Data Mitra C'3Daraaa</p>
        </div>
            ...
            <div class="mb-3">
                <form method="GET" action="{{ route('mitra.list') }}">
                    <div class="row">
                        ...
                        <div class="col-md-3">
                            <!-- Dropdown Tahun Bergabung -->
                            <select name="tanggal_bergabung" onchange="this.form.submit()" class="form-select">
                                <option value="">Pilih Tahun Bergabung</option>
                                @for ($i = 2024; $i >= 1900; $i--)
                                    <option value="{{ $i }}" {{ request('tanggal_bergabung') == $i ? 'selected' : '' }}>{{ $i }}</option>
                                @endfor
                            </select>
                        </div>
                        ...
                        <div class="col-md-4">
                            ...
                            <div class="input-group">
                                ...
                                @if (request('search'))
                                    <a href="{{ url()->current() }}" class="btn btn-outline-danger ml-3" id="clear-search">Clear</a>
                                @endif
                            </div>
                        </div>
                    </div>
                    ...
                    <tbody>
                        ...
                        @foreach ($dataMitra as $row)
                            <tr>
                                ...
                                <td>{{ $row->nomor_hp }}</td>
                                <td>
                                    @if ($row->jenis_kemitraan == 'Platinum')
                                        <span class="badge bg-info">Platinum</span>
                                    @elseif($row->jenis_kemitraan == 'Gold')
                                ...
                                <td>
                                    <a href="{{ route('mitra.edit', $row->mitra_id) }}" class="btn btn-warning btn-sm">
